...
sel_2 = random.randint(0, len(epochs)-1)
field = fields[sel_1]
epoch = epochs[sel_2]
field_trim = field.split('-').pop(0)
filename = field_trim + '/' + field + '.' + epoch + '.dat' 
time_vect = open(pathtotimevectors + filename).readlines()
tot_time = float(time_vect[-1]) - float(time_vect[0])
print '-- Time vector --' + '\n', filename
print 'Total time:', tot_time, '# Data points:', len(time_vect)

# Generate base names for all MOA data files.
allmoadata = []
